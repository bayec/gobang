...
function adjustSizeGen() {
    var t = navigator.userAgent.toLowerCase().match(/(iphone|ipod)/), e = $("#game-region"), n = $(".board td"),
        o = $(".go-board"), a = $("header.game-ult"), i = $("#game-info"), r = $("#main-but-group"),
        s = $("#other-but-group");
    return function () {
        var c, l = window.innerHeight, h = window.innerWidth, u = Math.max(l - 7, .98 * l),
            m = Math.max(h - 7, .98 * l), f = Math.min(u - 150, m), g = Math.min(m - 320, u - 40);
        t && (l > h ? (f = h, g = 0) : (g = l - 40, f = 0)), f > g ? (c = Math.min(~~((f - 15) / 15 / 2), ~~((h - 22) / 15 / 2)), e.css({
            ...
            left: 6,
            right: 6
        }), a.css("line-height", "80px"), i.css({top: 20, width: (15 * (2 * c + 1) + 12) / 2 - 150}), r.css({
            top: 6,
            right: 6,
            width: 160
        }), s.css({bottom: 6, right: 6, width: 160})) : (c = ~~((g - 15) / 15 / 2), e.css({
            padding: c + 6,
            ...
            left: 160,
            right: 160
        }), a.css("line-height", 36 + c + "px"), i.css({left: 15, top: 36 + c, width: 121 - c / 2}), r.css({
            top: 36 + c,
            right: 6,
            width: 160
...
function showWinDialog(t) {
    if (gameInfo.setBlinking(!1), "hvh" === t.mode) {
        ...
        $("#game-won h4").html(e + "赢了！"), gameInfo.value = e + " won.", $("#win-content").html(e + "赢了，再来一局吗？"), $("#happy-outer").fadeIn(500)
    } else t.getCurrentPlayer() instanceof HumanPlayer ? ($("#game-won h4").html("您赢了！"), $("#win-content").html("您赢了，再来一局吗？"), gameInfo.value = "You won.", $("#sad-outer").fadeIn(500)) : ($("#game-won h4").html("您输了！"), $("#win-content").html("您输给了电脑，再来一局吗？"), gameInfo.value = "Computer won.", $("#happy-outer").fadeIn(500))
}

Player.prototype.myTurn = function () {
    this.game.setCurrentColor(this.color), gameInfo.setText(function (t) {
        return t.charAt(0).toUpperCase() + t.slice(1)
    }(this.color) + "走"), gameInfo.setColor(this.color), gameInfo.setBlinking(!1)
    ...
    Player.prototype.myTurn.call(this), this.game.toHuman(this.color), this.other instanceof AIPlayer && gameInfo.setText("轮到你了！")
}, AIPlayer.prototype = new Player, AIPlayer.prototype.myTurn = function () {
    Player.prototype.myTurn.call(this), this.game.toOthers(), gameInfo.setText("思考中..."), gameInfo.setBlinking(!0), this.move()
    ...
    if (0 === this.game.rounds) this.setGo(7, 7); else if (1 === this.game.rounds) for (var t = [[6, 6], [6, 7], [6, 8], [7, 6], [7, 7], [7, 8], [8, 6], [8, 7], [8, 8]]; ;) {
        var e = Math.floor(Math.random() * t.length);
        if (this.setGo(t[e][0], t[e][1])) return;
        t.splice(e, 1)
    } else this.worker.postMessage({type: "compute"})
...
var Board = function (t, e) {
    ...
    function o(t, e, o, a) {
        if (a) for (o = 0; 2 > o; o++) for (var i = 0; 4 > i; i++) {
            ...
            for (var i = 0; 2 > i; i++) for (var d = 0; 4 > d; d++) h[i][d][t][e] = -o, n(t, e, o, i)
        }
    }
...
$(document).ready(function () {
    ...
    $(window).resize(e), e(), $.mobile.defaultDialogTransition = "flip", $.mobile.defaultPageTransition = "flip", $('#mode-select input[type="radio"]').on("change", function () {
        ...
        if ("vshuman" === gameData.mode) t.mode = "hvh", t.init(new HumanPlayer("black"), new HumanPlayer("white")); else {
            var n, o;
    ...
    }), $("#new-game").page(), $("#game-won").page(), gameData.load(), $(".back-to-game").button("disable"), $.mobile.changePage("#new-game", {changeHash: !1}), window.gameInfo = function () {
        var t = !1, e = "", n = "", o = {};
    ...
    prefix: "yyjhao.gomoku.", records: {}, addRecord: function (t, e, n) {
        this.records[t] = e;
        var o;
        o = n ? n : function () {
